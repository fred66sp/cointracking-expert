...
> 🔧 **Implicación para el framework:** para dar soporte al 721 se necesita **valoración de tenencias en EUR a 31/12** por activo y custodio, y la clasificación de cada cuenta como **residente / no residente** y **custodia / autocustodia**. Encaja con el motor de tenencias + valoración; candidato a un reporte específico.
...
Se citan aquí porque explican de dónde obtiene la AEAT información cruzada sobre las operaciones del contribuyente (relevante para la coherencia de datos y trazabilidad), pero **el framework, orientado al inversor persona física, no los genera**.
...
## 3. Relación con el resto del framework

- El Modelo 721 depende de **tenencias valoradas a 31/12** → motor de tenencias + fuente de precios EUR.
- Requiere metadato por cuenta: **jurisdicción del custodio** (residente/no residente) y **tipo** (custodia/autocustodia) → candidato a enriquecer `knowledge/exchanges/` y el modelo de cuentas.
...
3. Determinación del **valor a 31/12** (fuente y método de precio) — compartida con el motor fiscal.
