...
- **[official/CSV_FORMAT.md](official/CSV_FORMAT.md)** — Formato de la exportación "Trade Table", validado contra datos reales. Referencia autoritativa para la capa de importación (columnas, tipos, fechas, comisiones, emparejamiento de transferencias, colisión de tickers, duplicados).
- **[official/COST_BASIS_AND_VALIDATION.md](official/COST_BASIS_AND_VALIDATION.md)** — Cómo CoinTracking calcula la base de coste ("purchase pool") y detecta inconsistencias. Conocimiento destilado de fuentes oficiales, base para la auditoría (`tools/ct_audit.py`, skill `audit-cointracking`) y la preparación fiscal (skill `spanish-tax-return`).
...
- **[TROUBLESHOOTING.md](TROUBLESHOOTING.md)** — índice de enrutamiento por síntoma/warning (no añade conocimiento nuevo): ante un warning literal de CoinTracking o un síntoma descrito por el usuario, remite directamente al documento o caso (`knowledge/patterns/cointracking_casos_v2.yaml`) con la causa, evidencia y solución.
