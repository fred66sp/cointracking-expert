...
✅ **MCP sincronizado en caliente (ADR-016):** en cuanto quede fijado el proyecto activo, llama a `cointracking_switch_project(project_name=<proyecto>)` (si el MCP está conectado) antes de cualquier otra tool `cointracking_*`, para que su **caché** corresponda al proyecto activo — sin reiniciar el servidor ni tocar `.mcp.json`. Cada skill ya lo hace en su Paso -1. ⚠️ **Alcance de cuenta (ADR-040):** por defecto las credenciales son del proceso — todos los proyectos consultan la **misma cuenta** de CoinTracking y el switch aísla solo caché/datos. Para que un proyecto use **otra cuenta**, arranca el servidor con `--project-env-dir <dir>` y crea `<dir>/<proyecto>.env` (con `COINTRACKING_API_KEY`/`COINTRACKING_API_SECRET`; ese directorio **nunca** se versiona) — el switch entonces cambia también de cuenta, y su respuesta siempre indica cuál está activa (`credentials_source` + API key ofuscada). Sin `--project-env-dir`, no uses el switch para auditar dos cuentas distintas: contaminaría la caché con datos de la cuenta equivocada.
...
- **Commits:** en español, uno por cambio lógico. **Nunca hagas `git commit` ni `git push` sin consentimiento explícito del usuario:** aplica los cambios en los ficheros y **pide permiso antes** de commitear/pushear.
- **Gobernanza proactiva (ADR-042):** detectar que algo merece ADR es responsabilidad **del agente**, no del usuario. Ante un evento ADR-digno — hallazgo operativo repetible con diagnóstico no obvio, decisión de diseño tomada por chat, causa raíz que revela una regla vinculante, corrección del usuario que redefine cómo trabajar, o protocolo improvisado con éxito — **propón el ADR en el momento** (qué registrarías y por qué), **pide permiso** (es Categoría B, ADR-026) y créalo con el visto bueno (checklist ADR-030, frontmatter con `version:`, ambos índices, CHANGELOG). Proporcionalidad: si no encaja en esos 5 disparadores, no lo propongas; si ya lo cubre un ADR existente, propón actualizarlo, no duplicarlo.
