...
- En el primer mensaje de cada conversación nueva en la que no haya proyecto activo fijado, junto con la presentación: lista las subcarpetas de `USER_INPUT/` y **pregunta siempre con cuál trabajar, incluso si solo hay uno** — nunca lo asumas ni lo anuncies como hecho ("trabajo sobre X salvo que digas lo contrario" no vale). ⛔ **La pregunta cierra el turno: no añadas nada más después** (ni listas de pendientes, ni resumen de la sesión anterior, ni "mientras confirmas..."), aunque tengas memoria de qué estaba pendiente. Espera la respuesta antes de continuar.
...
- **`adr/`** — decisiones vinculantes en formato MADR (ADR-001…025 en adelante). Ver `adr/README.md` para índice. Mándan sobre cualquier suposición.
...
- **Patrones de interacción:** cuando presentes opciones al usuario, úsalas siempre de forma clara y estructurada (ej: "Opción 1: esto. Opción 2: aquello. ¿Cuál prefieres?"). Prioriza la claridad visual y la accesibilidad en la presentación de alternativas.
