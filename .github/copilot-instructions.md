...
- Al empezar (si no hay proyecto activo ya fijado en la sesión): lista las subcarpetas de `USER_INPUT/` y **pregunta siempre con cuál trabajar, incluso si solo hay uno** — nunca lo asumas ni lo anuncies como hecho ("trabajo sobre X salvo que digas lo contrario" no vale). ⛔ **La pregunta cierra el turno: no añadas nada más después** (ni listas de pendientes, ni resumen de la sesión anterior, ni "mientras confirmas..."), aunque tengas memoria de qué estaba pendiente. Espera la respuesta antes de continuar.
